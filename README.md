![banner](https://raw.githubusercontent.com/DavidFerreira21/DavidFerreira21/main/banner.jpg)


<h1 align="center">David Ferreira</h1>

<p align="center">
Cloud Platform Engineer | AWS | Terraform | Kubernetes | Platform Engineering
</p>

<p align="center">
<img src="https://skillicons.dev/icons?i=aws,terraform,kubernetes,python,linux,git"/>
</p>


Engenheiro de Plataforma Cloud com mais de 16 anos de experiência projetando, automatizando e operando ambientes corporativos em larga escala, tanto em ambientes On-Premises quanto em Cloud, com forte foco em AWS.

Minha atuação é bastante **hands-on**, focada em **Platform Engineering, Infrastructure as Code e automação de ambientes cloud escaláveis, resilientes e seguros**.

Trabalho principalmente com **Terraform** para provisionamento de infraestrutura e **Python** para automações voltadas à governança, padronização e operação de ambientes cloud.

Tenho experiência na construção e evolução de **plataformas cloud corporativas**, incluindo:

- Estratégias **multi-account com AWS Organizations e Control Tower**
- Implementação de **guardrails e governança**
- Segurança e observabilidade com **Security Hub e GuardDuty**
- Padronização de infraestrutura com **Infrastructure as Code**
- Apoio a times de engenharia e DevOps em **deploys, troubleshooting e evolução de plataformas**

---


# 🚀 Tecnologias

![AWS](https://img.shields.io/badge/AWS-Cloud-orange?logo=amazonaws)
![Terraform](https://img.shields.io/badge/Terraform-IaC-purple?logo=terraform)
![Kubernetes](https://img.shields.io/badge/Kubernetes-Orchestration-blue?logo=kubernetes)
![Python](https://img.shields.io/badge/Python-Automation-yellow?logo=python)
![GitOps](https://img.shields.io/badge/GitOps-Deployment-black?logo=git)
![Linux](https://img.shields.io/badge/Linux-Platform-grey?logo=linux)

---

# 🏅 Certificações

- ☁️ **AWS Certified Solutions Architect – Associate**
- ☁️ **AWS Certified SysOps Administrator – Associate**
- 🖥 **VMware VCP 6.7**
- 🪟 **MCSA Windows Server**
- 📧 **MCSA Exchange Server**

---

# 📦 Projetos em Destaque

### 🕒 K8s OffHours
Toolkit para **redução de custos em clusters Kubernetes**, automatizando o desligamento de workloads fora do horário comercial.

🔗  
https://github.com/DavidFerreira21/k8s-offhours


---

### 🏗 API Account Factory
Projeto de automação para **criação e padronização de contas AWS**, seguindo boas práticas de governança e estrutura organizacional.

🔗  
https://github.com/DavidFerreira21/api-account-factory

---

# ✍️ Blog Técnico

Escrevo sobre **Cloud Architecture, Terraform, Kubernetes, Platform Engineering e governança de ambientes cloud**.

📖  https://davidferreira.blog



---


# 📫 Contato

🔗 LinkedIn  
https://linkedin.com/in/davidferreirams  

📝 Blog  
https://davidferreira.blog  

💻 GitHub  
https://github.com/DavidFerreira21


<p align="center">
<img src="https://skillicons.dev/icons?i=aws,terraform,kubernetes,python,linux,git"/>
</p>
